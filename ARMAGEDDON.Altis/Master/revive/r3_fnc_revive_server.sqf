...
if(!isNull player) then {
	...
	{
		private ["_unit", "_killer", "_wound", "_handleDamage", "_killer", "_unitIsDown","_handleNewDamage"];
		...
		if(r3_enableDebugMode) then { diag_log format ["HandleDamage for AI-UNIT: %1, Wound: %2, Damage: %3, Source: %4, Owner: %5", _unit, _wound, _handleDamage, _killer, (owner _unit)]; };
		
		if(local _unit) then {
			if(alive _unit && ((damage _unit) + _handleDamage) >= 1 && _unitIsDown == 0) then {
				...
				if(damage _unit >= 0.4 && _unitIsDown == 0) then {
					if("FirstAidKit" in (items _unit) OR "Medikit" in (items _unit) ) then {
						[_unit] spawn r3_aiHealingSelf;
					};
				} else {
					if(_unitIsDown == 0) then { _handleNewDamage = (_handleDamage / 2) };
				};
			};
		};
		_handleNewDamage
	...
	r3_spawnFakeUnit_AI =
	{
		...
		if(!isNil {goggles _unit}) then {_fakePlayer addGoggles (goggles _unit)} else {removeGoggles _fakePlayer};
	...
	r3_unconscious_ai =
	{
		...
		_txt = format["%1: Damn, im down !",name _unit];
		[[_unit, _txt], "groupChatMP"] call BIS_fnc_MP;
		
		waitUntil{sleep 1; (getPosATL _fakePlayer select 2) < 1 OR speed _fakePlayer == 0};
		...
		if(_aiMedic getVariable ["r3_unitIsCalled",0] == 0) then {
			if(isNull (_unit getVariable ["r3_unitPrivateMedic", objNull])) then {
				if(alive _unit && _unit getVariable ["r3_unitIsDown",0] == 1) then {
					...
					_oldGRP = group _aiMedic;
					_medicGRP = createGroup (side _aiMedic);
					[_aiMedic] joinSilent _medicGRP;
					
					_aiMedic reveal _unit;
					_aiMedic allowDamage false;
					_aiMedic stop false;
					_aiMedic setCaptive true;
					_aiMedic disableAI "AUTOTARGET";
					_aiMedic disableAI "TARGET";
					_aiMedic setBehaviour "CARELESS";
					_aiMedic setCombatMode "BLUE";
					_aiMedic move (getPos _unit);
					_aiMedic setSpeedMode "FULL";
					
					_aiMedic doMove (getPos _unit);
					_aiMedic moveTo (getPos _unit);
					...
					while{(_aiMedic distance _unit) >= 4 OR time > _moveTimeout} do {
						if(!alive _unit OR !alive _aiMedic) exitWith {};
						sleep 0.2;
						if(_unit getVariable ["r3_unitIsDown",0] == 0) exitWith {};
						sleep 0.2;
						if(_aiMedic getVariable ["r3_unitIsDown",0] == 1) exitWith {};
						sleep 0.2;
						if(_unit getVariable ["r3_unitGetDrag",0] == 1) exitWith {};
						sleep 0.2;
						if(_unit getVariable ["r3_unitGetRevive",0] == 1) exitWith {};
						sleep 0.2;
					};

					if(alive _unit && alive _aiMedic) then {
						if((_aiMedic distance _unit) <= 5) then {
							if(_unit getVariable ["r3_unitGetRevive",0] == 0) then {
								if(_aiMedic getVariable ["r3_unitIsDown",0] == 0) then {
									if(_unit getVariable ["r3_unitGetDrag",0] == 0) then {
					...
					_aiMedic setBehaviour (behaviour (leader _oldGRP));
					_aiMedic setCombatMode (combatMode _oldGRP);
					sleep 0.5;
					[_aiMedic] joinSilent _oldGRP;
