...
if(!isNull player) then {
	...
	r3_changeRevCamPos =
	{
		...
		if(!isNull r3_revCam) then {
			if((count r3_revCamPos1) > 0 && (count r3_revCamPos2) > 0 && (count r3_revCamPos3) > 0) then {
				if(_way == 1) then {
					if(r3_revCamInt == 3) then {
						r3_revCamInt = 1;
					} else {
						r3_revCamInt = r3_revCamInt +1;
					};
				} else {
					if(r3_revCamInt == 1) then {
						r3_revCamInt = 3;
					} else {
						r3_revCamInt = r3_revCamInt -1;
					};			
	...
	{	
		...
		if (time > _bleedOut && player getVariable ["r3_unitIsStabi",0] == 0 && player getVariable ["r3_unitIsDown",0] == 1) then {
			[]call r3_playerSuicide;
			hint"Du bist verblutet.";
		};	
	...
	r3_handleDrag = 
	{
		...
		if(!isNull _originalTarget) then {	
			if(alive _originalTarget && _target isKindOf "Man") then {
				...
				_releaseId = player addAction ["<img image='Master\img\release32.paa'/> <t color='#C90000'>Release</t>", {r3_playerIsDragging = 0}, [], 10, true, true, "", "r3_playerIsDragging == 1"];
	...
	r3_playerSuicide = 
	{
		...
		closeDialog 0; life_respawned = true; [] call life_fnc_spawnMenu;
	...
	r3_callForMedic = 
	{
		...
		if(r3_MedicAlreadyCalled)then{hint"Warte bitte 2 Minuten bevor du erneut einen Sanitäter rufen kanns!"};
