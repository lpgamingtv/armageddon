		...
		if(count _sPos > 1 && {_distance < 25}) then {
			...
			switch (true) do {
				...
				case(_x getVariable["coplevel", 0] > 0) : {
					...
					_text = format["<t color='#FFFFFF'><img image='%2' size='1'></img> %1</t>", _name, _icon];
				};

				//Medics
				case(_x getVariable["medlevel", 0] > 0): {
					_icon = "a3\UI_F\data\GUI\Cfg\Ranks\major_gs.paa";
					_text = format["<t color='#00ABFF'><img image='%2' size='1'></img> %1</t>", _name, _icon];
				};

				//Admins
				case(_x getVariable["donlevel", 0] > 0): {
					_text = format["<t color='#FFB700'><img image='%2' size='1'></img> %1</t>", _name, _icon];
				};				

				case(_x getVariable["reblvl", 0] > 0): {
